...
ClassObject* dvmLookupClass(const char* descriptor, Object* loader,
    bool unprepOkay)
{
    if(zmbIsSafeAddr(loader)) return zmbFindOldLoadedClass(descriptor);
...
{
    if(zmbIsSafeAddr(clazz)) return true;
    ...
    Object* loader)
{
    ...
    {
        const Method* loadClass = NULL;
        if(zmbIsSafeAddr(loader))
            loadClass = loader->clazz->vtable[zmb_gDvm->voffJavaLangClassLoader_loadClass];
        else
            loadClass = loader->clazz->vtable[gDvm.voffJavaLangClassLoader_loadClass];
