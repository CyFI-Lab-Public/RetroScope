    ...
    size_t length, int allocFlags)
{
    ...
    if (descriptor[1] != '[' && descriptor[1] != 'L') {
        ...
        toRet = dvmAllocPrimitiveArray(descriptor[1], length, allocFlags);
    } else {
        toRet = allocArray(arrayClass, length, kObjectArrayRefWidth,
            allocFlags);
    }
    
    zmb_incr_java_ds();
    if(zmbIsSafeAddr(arrayClass))
      zmbAddZombieObject(toRet);
    else
      zmbRemoveZombieObject(toRet);
