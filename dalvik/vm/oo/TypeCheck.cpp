    ...
    const ClassObject* clazz)
{
  ...
  if (zmbIsSafeAddr(instance) && !zmbIsSafeAddr(clazz))
  {
    return isInstanceof(instance, (const ClassObject *)zmbFindOldLoadedClass(clazz->descriptor));
  }
  if (!zmbIsSafeAddr(instance) && zmbIsSafeAddr(clazz))
  {
    return isInstanceof((const ClassObject *)zmbFindOldLoadedClass(instance->descriptor), clazz);
  }
