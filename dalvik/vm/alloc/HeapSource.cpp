...
//    if (zmb_status) zmbAppMemAdd((void *)ptr, mspace_usable_size(ptr) + HEAP_SOURCE_CHUNK_OVERHEAD);
...
//    if (zmb_status) zmbAppMemRemove((void *)ptr, delta);
...
/*
#include "zombie/Zombie_Internal.h"

bool zmbIsGoodHeapSource(void* test) {
  void** test_heap = (void**)test;

  static const char heap_name[] =  "[heap]";
  MemSeg* seg = zmbMemSegForAddr(*test_heap);
  if(seg == NULL ||
     strcmp(seg->name, heap_name) != 0)
    return false;
  
  HeapSource* gHs_ptr = (HeapSource*)zmbTranslateAddrInSeg(*test_heap, seg);
  if(gHs_ptr->liveBits.base == 0 ||
     gHs_ptr->liveBits.base != gHs_ptr->markBits.base ||
     (void*)gHs_ptr->liveBits.base != gHs_ptr->heaps[gHs->numHeaps-1].base ||
     (void*)gHs_ptr->markBits.base != gHs_ptr->heaps[gHs->numHeaps-1].base)
    return false;

  static const char dal_heap_name[] =  "dalvik-heap";
  seg = zmbMemSegForAddr(gHs_ptr->heaps[0].base);
  if(seg == NULL ||
     (void*)seg->start != (void*)gHs_ptr->heaps[gHs->numHeaps-1].base ||
     strstr(seg->name,  dal_heap_name) == NULL)
    return false;

  return true;
}
void zmbEatHeapSourceBrains(void* brains_ptr, DvmGlobals* gDvm_ptr) {
  void** hs = (void**)brains_ptr;
  HeapSource* gHs_ptr = (HeapSource*)zmbTranslateAddr(*hs);
  *gHs = *gHs_ptr;
}
*/
